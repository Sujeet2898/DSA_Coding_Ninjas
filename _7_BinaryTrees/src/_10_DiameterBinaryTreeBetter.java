public class _10_DiameterBinaryTreeBetter {
}
...
public class Pair<T,V>{
    public T first;     // first means Height
    public V second;    // second means Diameter
}
----------------------------------------------
...
public class BinaryTreeUse {
    ...
    public static Pair<Integer, Integer> heightDiameter(BinaryTreeNode<Integer> root){
        ...
        // Call recursively on "Left" to bring both height & diameter simultaneously
        // This leftOutput contains both leftHeight and leftDiameter
        Pair<Integer, Integer> leftOutput = heightDiameter(root.left);

        // Call recursively on "Right" to bring both height & diameter simultaneously
        // This rightOutput contains both rightHeight and rightDiameter
        Pair<Integer, Integer> rightOutput = heightDiameter(root.right);

        // Finding overAll height
        int height = 1 + Math.max(leftOutput.first, rightOutput.first);

        // Finding overAll diameter which has three options
        // 1. dia = sum of Height of left & Height of right
        int option1 = leftOutput.first + rightOutput.first;

        // 2. dia = diameter of left
        int option2 = leftOutput.second;

        // 3. dia = diameter of right
        int option3 = rightOutput.second;

        // So, overAll diameter is maximum of option1 with maximum of option2 & option3
        int diameter = Math.max(option1, Math.max(option2, option3));

        // We have to return overAll height and overAll diameter as output
        Pair<Integer, Integer> output = new Pair<>();
        output.first = height;
