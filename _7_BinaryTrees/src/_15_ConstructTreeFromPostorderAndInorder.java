public class _15_ConstructTreeFromPostorderAndInorder {
}
...
public class BinaryTreeUse {
    ...
    public static BinaryTreeNode<Integer> buildTreesHelper(int in[], int post[], int inS, int inE, int postS, int postE){
        ...
        // rootData is at the ending of postOrder and work is to be done within range ( not "post.length - 1" )
        ...
        // 4 For PostOrder
        int leftPostS = postS;
        int leftPostE = (leftInE - leftInS) + leftPostS;   // leftPostE - leftPostS = leftInE - leftInS
        int rightPostS = leftPostE + 1;
        int rightPostE = postE - 1;

        // Recursively call on left subTree and attached to left of root
        root.left = buildTreesHelper(in, post, leftInS, leftInE, leftPostS, leftPostE);

        // Recursively call on right subTree and attached to right of root
        root.right = buildTreesHelper(in, post, rightInS, rightInE, rightPostS, rightPostE);

        return root;
    }

    public static BinaryTreeNode<Integer> buildTrees(int in[], int post[]){
        return buildTreesHelper(in, post, 0, in.length - 1, 0, post.length - 1);
    }

    public static void main(String[] args) {
        int in[] = {4, 2, 5, 1, 6, 3, 7};
        int post[] = {4, 5, 2, 6, 7, 3, 1};
        BinaryTreeNode<Integer> root = buildTrees(in, post);
        printLevelWise(root);
    }
}
----------------------------------------------
