...
public class Pair<T,V>{
    public T first;     // first means head
    public V second;    // second means tail
}
----------------------------------------------
...
public class BinaryTreeUse {
    ...
    public static Pair<LinkedListNode<Integer>,LinkedListNode<Integer>> BSTToSortedLL(BinaryTreeNode<Integer> root){

        // Basecase
        if( root==null ) {
            Pair<LinkedListNode<Integer>,LinkedListNode<Integer>> output = new Pair<LinkedListNode<Integer>, LinkedListNode<Integer>>();
            output.first = null;      // first i.e head is null
            output.second = null;     // second i.e tail is null
            return output;
        }

        // Call left & right
        Pair<LinkedListNode<Integer>,LinkedListNode<Integer>> leftOutput = BSTToSortedLL(root.left);
        Pair<LinkedListNode<Integer>,LinkedListNode<Integer>> rightOutput = BSTToSortedLL(root.right);

        // Constructing fresh node for LinkedList
        LinkedListNode<Integer> newNode = new LinkedListNode<Integer>(root.data);

        // Applying condition
        if(leftOutput.second != null)
            leftOutput.second.next = newNode;
        else
            leftOutput.first = newNode;

        if(rightOutput.first != null)
            newNode.next = rightOutput.first;
        else
            rightOutput.second = newNode;

        // Return
        Pair<LinkedListNode<Integer>,LinkedListNode<Integer>> output = new Pair<LinkedListNode<Integer>, LinkedListNode<Integer>>();
        output.first = leftOutput.first;         // returning overall head
        output.second = rightOutput.second;      // returning overall tail
        return output;
    }
